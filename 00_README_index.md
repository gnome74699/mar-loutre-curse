# 🦦 EVG Yoann Louis — 1er Mai 2026 à Lyon
## Index des livrables

---

## 📚 Les 3 fichiers à utiliser

### 1. `antidote_marie_loutre_v13.md` — LE SCÉNARIO COMPLET
**Pour qui :** les 3 témoins (toi + Brice + Jonathan), document de référence.
**Contient :** synopsis, mécaniques (Esprits Diaboliques + Garde du Roi), planning détaillé, parcours des 10 étapes avec énigmes/défis/actions diaboliques par étape, briefings, conseils sécurité, recettes mâchon et cocktail final.
**À lire :** au moins 1 fois en intégralité dans la semaine qui précède.

---

### 2. `garde_du_roi_fiches_roles_v4.md` — FICHES À PIOCHER À BELLECOUR
**Pour qui :** les 6 invités, distribuées **au tirage au sort à 14h00 sous le cheval de Louis XIV**.
**Contient :** les 6 fiches rôles façon Naheulbeuk (Barbare / Elfe / Nain / Mage / Ranger / Voleur), chacune avec son nom de personnage, son handicap RP et son pouvoir unique (1 utilisation).
**À faire :**
1. Imprimer le doc
2. Découper les 6 fiches (1 par page, format A5 conseillé)
3. Plier chaque fiche en deux pour la pioche
4. Glisser les 6 fiches pliées dans un sac en tissu opaque
5. Apporter à Bellecour à 14h pour le tirage au sort

⚠️ **Ne PAS distribuer aux invités à l'avance** — la surprise sur place fait partie du jeu.

---

### 3. `esprits_diaboliques_fiches_memo_v5.md` — FICHES MÉMO TÉMOINS
**Pour qui :** vous trois, une fiche par témoin (Pacte / Mensonge / Tentation), à garder en poche pendant les 4h.
**Contient :** mécanique de chaque pouvoir, règles, exemples par étape, planning, et pour Brice le tableau des **10 kits d'indices Mensonge** prêts à l'emploi.
**À faire :**
1. Imprimer le doc
2. Découper en 3 sections (Pacte / Mensonge / Tentation)
3. Chaque témoin garde la sienne en poche (la fiche Mensonge fait 2 pages — pochette ou mini-classeur format poche)

---

## 🗓️ Planning J-jour en 1 coup d'œil

| Heure | Phase |
|---|---|
| 13h05 | 3 témoins arrivent chez Yoann |
| 13h05–13h30 | Transformation + Énigme du Ralliement (4 cartes) |
| **13h30** | **Départ en voiture / VTC vers Bellecour** (30 min) |
| **14h00** | **Convergence à Bellecour** sous le cheval Louis XIV |
| 14h00–14h15 | Cérémonie pioche + 10 min lecture des rôles |
| 14h15–14h30 | Présentations solennelles + défi de la queue |
| 14h30 | Étapes 2-9 (Saint-Ex → Terreaux → Pradel → Sathonay → Gros Caillou → Saône → Traboule → Cathédrale) |
| 17h00 | Pied du Calvaire de la Loutre (Montée Saint-Barthélemy) |
| 17h20 | Esplanade Fourvière — cocktail antidote |
| 17h40+ | Mâchon Royal Lyonnais — durée libre |

---

## 🛒 Check-list rapide à faire avant le J-jour

**Imprimés :**
- [ ] Les 6 fiches rôles Garde (à plier pour la pioche)
- [ ] Les 3 fiches mémo Esprits (Pacte / Mensonge / Tentation)
- [ ] Les 4 cartes de l'Énigme du Ralliement (à mettre dans une enveloppe rouge cachetée)
- [ ] Une carte papier de Lyon avec le parcours surligné (en cas de panne smartphone)

**Costume + accessoires :**
- [ ] Le costume de loutre intégral
- [ ] 6 accessoires identifiants Garde : bandeau rouge / foulard ivoire / chope plastique opaque / foulard violet + bâton / casquette / bandana noir + petit sac
- [ ] 2 sacs en tissu opaque (un pour les fiches, un pour les accessoires)

**Ingrédients de l'Antidote :**
- [ ] Fer à cheval (chocolat de préférence)
- [ ] Fiole de grenadine ("larmes de lion")
- [ ] Médaillon "Louis"
- [ ] Plume
- [ ] Brin de muguet sacré (à planquer pour Fourvière, en cas de récolte ratée)

**Sac à Dos Gourmand (à acheter la veille) :**
- [ ] Rosette de Lyon
- [ ] Jésus de Lyon
- [ ] Grattons lyonnais (paquet)
- [ ] Brioche aux pralines roses
- [ ] Saint-Marcellin
- [ ] Saucisson brioché
- [ ] Cervelle de canut
- [ ] Tablier de sapeur (si trouvé)
- [ ] Coussins de Lyon (chocolats verts)
- [ ] Tarte praline rose
- [ ] Baguettes / pain de campagne
- [ ] Bouteille Côtes du Rhône ou Beaujolais

**Glacière finale :**
- [ ] Rhum + jus d'ananas + grenadine + jus de citron vert + Tabasco
- [ ] Shaker (ou bouteille avec couvercle qui se ferme)
- [ ] 9 verres en plastique solide
- [ ] Glaçons en sac isotherme
- [ ] Briquet (pour effet flambé optionnel)
- [ ] Couteaux à charcuterie + planche en bois
- [ ] Nappe (pour le mâchon)
- [ ] Eau (beaucoup) + essuie-tout

**Logistique :**
- [ ] **VTC réservé la veille pour 13h30** (transfert chez Yoann → Bellecour)
- [ ] Vérifier la veille au soir le trajet du cortège syndical du 1er mai (Le Progrès / Lyon Capitale)
- [ ] Vérifier l'ouverture de la Longue Traboule (rue du Bœuf 27)
- [ ] Vérifier les horaires de l'horloge astronomique cathédrale Saint-Jean
- [ ] Briefing aux invités envoyé (cf. message-type dans le scénario v13)
- [ ] Téléphone dédié aux photos/vidéos chargé à 100%
- [ ] Argent liquide pour le muguet et imprévus
- [ ] Brins de muguet de secours (5 minimum) planqués dans le sac, au cas où la récolte rate

---

*Bon courage les gars. Et longue vie à Loutre-Yoann.* 🦦💍🌸🥓
